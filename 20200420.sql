table 에는 저장순서가 없다. / 조회순서가 없다
==> ORDER BY 컬럼명 정렬방식, .... : 컬럼의 순서가 바뀌거나, 컬럼 추가가 되면 원래의도대로 동작하지 않을 가능성이 있음
...
2. 1번부터 순차적으로 읽는 경우만 가능
 ...
 ROWNUM의 실행순서에 의해 정렬이 된상태로 ROWNUM 을 부여하면 IN-LINE VIEW를 사용해야 한다.
 ** IN-LINE : 직접 기술을 했다;
 ...
 INLINE-VIEW와 비교를 위해 VIEW 를 직접 생성(선행학습, 나중에 나온다)
 VIEW - 쿼리
...
**신규문제
PROD 테이블을 PROD_LGU(내림차순), PROD_COST(오름차순)으로 정렬하여 페이징 처리 쿼리를 작성하시오
단 페이지 사이즈는 5 바인드 변수 사용한것
  ...
  FROM 
    (SELECT ROWNUM rn, a.*
      FROM 
        (SELECT *
          FROM prod
         ORDER BY prod_lgu, prod_cost DESC) a)
    WHERE rn BETWEEN 1 + (:page - 1) * :pageSize AND :page * :pageSize;
