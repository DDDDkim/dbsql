...
크게 다음 두가지 형태로 사용
1. 테이블의 모든 컬럼의 값을 입력하는 경우, 컬럼명을 나열하지 않아도 된다.
...
2. 입력하고자 하는 컬럼을 명시하는 경우
    ...
    단 테이블에 NOT NULL 설정이 되어있는 컬럼이 누락되면 INSERT 는 실패한다.
...
INSERT INTO emp (empno, ename, hiredate) VALUES (9999, '김동훈', SYSDATE);

9998번 사번으로 jw 사원을 입력, 입사일자는 2020년 4월 13일
INSERT INTO emp (empno, ename, hiredate) VALUES (9998, 'jw', TO_DATE('200413', 'YY/MM/DD'));

empno 컬럼은 NOT NULL
INSERT INTO emp (ename, hiredate) VALUES ('jw', TO_DATE('200413', 'YY/MM/DD')); ==> 안됨
...
위에 작업한 INSERT 구문을 취소
ROLLBACK;



UPDATE : 데이터 수정
UPDATE 테이블명 SET 수정할 컬럼1 = 수정할 값 1,
                                    [수정할 컬럼1 = 수정할 값 1,.......]
[WHERE condition-SELECT 절에서 배운 WHERE 절과 동일
                수정할 행을 인식하는 조건을 기술]
...
UPDATE, DELETE : 기존에 있는 걸 변경, 삭제
    ==> 쿼리를 작성할 경우 주의
            ...
            2. UPDATE, DELETE 문을 실행하기전에 WHERE 절을 복사해서 SELECT를 하여
            영향이 가는 행을 눈으로 확인;
...
deptno, job 두개의 컬럼을 SIMTH 사원의 정보와 동일하게 변경
...
1. 특정 컬럼만 값을 삭제 ==> 해당 컬럼을 NULL로 UPDATE
    DELETE 절은 행 자체를 삭제
2. UPDATE와 마찬가지로 DELETE 쿼리를 실행하기 전에 SELECT를 통해 삭제 대상이되는 행을 조회, 확인하자.
...
WHERE 절을 사용 가능한 DML : SELECT , UPDATE, DELETE
    ...
    데이터를 식별하는 속도에 따라 쿼리의 실행 성능이 좌우 된다.
...
트랜잭션의 종료는 ROLLBACK, COMMIT 을 통해 종료가 된다.

트랜잭션 종료후 새로운 DML이 실행되면 새로운 트랜잭션이 시작


평소 사용하는 게시판을 생각해보자
게시글 입력할 때 입력하는 것 : 제목(1개), 내용(1개), 첨부파일(복수 가능)
...
즉 두개중에 하나라도 문제가 생기면 일반적으로 ROLLBACK 을 통해 두 개의 INSERT 구문을 취소.
