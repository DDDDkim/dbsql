...
SELECT LOWER('Hello, World'), UPPER('Hello, World'),  INITCAP('hello, world')
  ...
  emp테이블의 SMITH사원의 이름은 대문자로 저장되어 있음
  
  SELECT *
   FROM emp
   --WHERE LOWER(ename) = 'SMITH';이럭식으로 작성하면 안된다
   WHERE ename = UPPER('SMITH'); 두가지방식중에는 위에보다는 아래방식이 올바른 방식이다.
   WHERE sname = 'smith'; 테이블에는 데이터 값이 대문자로 저장되어 있으므로 조희건수 0
   WHERE sname = 'SMITH'; 정상실행
...
SUBSTR(문자열, 시작인덱스, 종료인덱스) : 문자열의 시작인덱스 부터.... 종료인덱스까지의 부분 문자열
...
SELECT SUBSTR('Hello, World', 1,5) sub,
        ...
        INSTR('Hello, World', 'o', 6) ins2, --(세번쨰인자 시작인덱스)        
...
ROUND(105.54, 1): 소숫점 첫번째자리까지 결과를 생성 ==> 소수점 두번째 자리에서 반올림
...
TO_DATE(문자열, 포맷) : 문자열을 포맷에 맞게 해석하여 날짜 타일으로 형변환
TO_CHAR(날짜, 포맷) : 날짜타입을 포맷에맞게 문자열로 변환
YYYY : 년도
MM : 월
DD : 일자
...
현재시간 시분초 단위까지 표현 ==> TO_CHAR를 이용한 형변환
